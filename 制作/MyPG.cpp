...
namespace MyPG
{
	...
	void MyGameEngine::CreateEffect(int no, ML::Vec2 pos) {
		...
		case 0: //上から落ちてくる10個の玉
			for (int i = 0; i < 10; i++) {
				if (auto e = Effect::Object::Create(true)) {
					//利用するファイル
					...
					e->gravityVec = ML::Vec2(0.f, GetRandom(0.1f, 3.f));
		...
		case 5://clear 画面中央　
			if (auto e = Effect::Object::Create(true)) {
				e->filename = "data/image/text/clear.png";
				e->resSizeX = e->drawSizeX = 480;
				e->resSizeY = e->drawSizeY = 147;
			...
			break;
		case 6://allclear 画面中央　
			if (auto e = Effect::Object::Create(true)) {
				e->filename = "data/image/text/congratulations.png";
				e->resSizeX = e->drawSizeX = 480;
				e->resSizeY = e->drawSizeY = 117;
				e->pos.x = (float)ge->screen2DWidth / 2;
				e->pos.y = (float)ge->screen2DHeight / 3;
				e->scale = 0;
				e->addScale = 0.05f;
				e->maxScale = 1.5f;
		...
		case 95://リセットフェイド 90flame
			if (auto e = Effect::Object::Create(true)) {
				...
				e->addAngle = 3.f;
				e->angle = 0.f;
				e->fadeMaxAngle = 180.f;
				e->alpha = 0.f;
			}
			break;
		case 96: //フェイドイン 45flame
			if (auto e = Effect::Object::Create(true)) {
...
*/
	...
	bool MyGameEngine::Initialize(HWND wnd_)
	{
		...
		auto startTask = Title::Object::Create(true);
		CreateEffect(98, ML::Vec2(0, 0));
